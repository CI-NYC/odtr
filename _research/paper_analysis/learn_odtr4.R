...
d <- odtr(observed, A, Y, W, L, learners1, learners1, learners2, filters, 1, "binomial")
d_cv <- odtr(observed, A, Y, W, L, learners1, learners1, learners2, filters, 10, "binomial")

# evaluate ODTR -----------------------------------------------------------

shifted <- as.data.frame(observed)
shifted[, A] <- d_cv$A_opt

theta_odtr <- lmtp_sdr(as.data.frame(observed),
                       A, Y, W, L,
                       shifted = shifted,
...
theta_odtr_wocf <- lmtp_sdr(as.data.frame(observed),
                            A, Y, W, L,
                            shifted = shifted,
                            outcome_type = "survival",
                            folds = 1,
...
condA <- observed[, glue("wk{2:tau}.use_this_week")] == 1

# Was dose under the allowable maximum? 
condB <- matrix(FALSE, dim(condA)[1], dim(condA)[2])

condB[observed$medicine == "bup", ] <- 
    observed[observed$medicine == "bup", glue("wk{2:tau}.dose_this_week")] < 32

# Was dose under the dose threshold? 
condC <- matrix(FALSE, dim(condA)[1], dim(condA)[2])
condC[observed$medicine == "bup", ] <- 
    observed[observed$medicine == "bup", glue("wk{2:tau}.dose_this_week")] < 16
...
dynamic <- as.data.frame(observed)
dynamic[, A] <- apply(condA & condB, 2, function(x) as.numeric(x))

theta_rudolph <- lmtp_sdr(as.data.frame(observed),
                          A, Y, W, L,
                          shifted = dynamic,
                          outcome_type = "survival",
                          folds = 1,
...
theta_constant <- lmtp_sdr(as.data.frame(observed),
                           A, Y, W, L,
...
saveRDS(res, here(analysis_root, glue("data/drv/odtr_dSL_{med}_cf.rds")))
