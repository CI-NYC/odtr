...
W <- names(dat)[-1]
L <- lapply(2:11, function(x) c(glue("wk{x-1}.dose_this_week"), glue("wk{x}.use_this_week")))
Y <- glue("wk{3:12}.relapse_this_week")

learners <- c("glm", "glmnet", "earth", "lightgbm", "nnet")
...
d <- odtr(dat, sem, 1, learners, learners, "binomial")
plan(sequential)

saveRDS(d, glue("data/drv/optimal-rule-050823-{id}.rds"))
